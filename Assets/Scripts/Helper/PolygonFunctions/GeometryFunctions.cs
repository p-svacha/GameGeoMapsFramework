...
public static class GeometryFunctions
{
    ...
    /// <summary>
    /// Returns the (unsigned) minimum distance from 'point' to any point on the line defined by linePoints.
    /// If isClosed is true, it is treated as a closed polygon, meaning the last point is connected to the first.
    /// </summary>
    public static float DistanceToLine(List<Vector2> linePoints, Vector2 point, bool isClosed)
    {
        if (linePoints == null || linePoints.Count == 0) return float.PositiveInfinity;

        int n = linePoints.Count;
        float minDistSqr = float.PositiveInfinity;

        int limit = isClosed ? linePoints.Count : linePoints.Count - 1;

        for (int i = 0; i < limit; i++)
        {
            Vector2 a = linePoints[i];
            Vector2 b = linePoints[(i + 1) % n];

            Vector2 ab = b - a;
            float abLenSqr = Vector2.Dot(ab, ab);

            float t = 0f;
            if (abLenSqr > 1e-8f)
            {
                t = Mathf.Clamp01(Vector2.Dot(point - a, ab) / abLenSqr);
            } // else: degenerate edge (a==b), closest is 'a'

            Vector2 closest = a + t * ab;
            float dSqr = (point - closest).sqrMagnitude;

            if (dSqr < minDistSqr) minDistSqr = dSqr;
        }
