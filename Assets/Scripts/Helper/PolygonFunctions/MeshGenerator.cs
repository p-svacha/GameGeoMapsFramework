...
public class MeshGenerator
{
    public static GameObject GeneratePolygon(List<Vector2> vertices2D, string objectName = "")
    {
        ...
        for (int i = 0; i < vertices.Length; i++)
        {
            ...
            */
            ...
            tangents[i] = new Vector4(tangent.x, tangent.y, 0, 0);
        }
        ...
        string objName = objectName == "" ? $"Polygon ({ vertices2D.Count})" : objectName;
        GameObject polygon = new GameObject(objName);
        MeshRenderer renderer = polygon.AddComponent<MeshRenderer>();
        renderer.material = ResourceManager.LoadMaterial("Materials/PolygonMaterial");
        Color ranCol = Color.red; // new Color(Random.Range(0f, 0.3f), Random.Range(0.4f, 1f), Random.Range(0.1f, 0.6f));
        renderer.material.color = new Color(0.41f, 0.53f, 0.23f);
    ...
    /// <summary>
    /// Creates a GameObject with a mesh that represents the bounds an AreaFeature. onOutside means the border will be drawn on the outside of the polygon.
    /// </summary>
    public static GameObject CreateSinglePolygonBorder(List<Vector2> points, float width, Color c, bool clockwise = false)
    {
        ...
        for (int i = 0; i < outerVertices.Count; i++)
        {
            vertices[2 * i] = new Vector3(outerVertices[i].x, outerVertices[i].y, 0);
            vertices[2 * i + 1] = new Vector3(innerVertices[i].x, innerVertices[i].y, 0);
        }
